...
//INICIO CUSTOM. - Thiago Meschiatti FSW - 27/01/16
//#DEFINE MAXITEMC   038                                                // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXITEMC   033                                                // Máxima de caracteres por linha de produtos/serviços
//FIM CUSTOM.
...
For nZ := 1 To nLenDet
	...
	//INICIO CUSTOM - CASSIANO
	//nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 50)
	nMaxCod := MaxCod(oDet[nX]:_Prod:_cProd:TEXT, 65)
	//INICIO CUSTOM - CASSIANO
	...
	If (Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U" .Or. Type("oNf:_infnfe:_det:_Infadprod:TEXT") <> "U") .And. ( lImpAnfav .Or. lImpInfAd )
		...
		EndDo
	EndIf
	//INICIO CUSTOM CASSIANO EM 05-06-17
//	If lPontilhado
//		aadd(aItens,{;
//			"-",;
//			"-",;
...
//		Endif
//	EndIf
	//FIM CUSTOM CASSIANO EM 05-06-17
...
//		cAux := SubStr(cAux,IIf(EspacoAt(cAux, MAXMENLIN) > 1, EspacoAt(cAux, MAXMENLIN), MAXMENLIN) + 1)
//	EndDo
	//INICIO CUSTOM. Thiago Meschiatti - FSW TOTVS IP Campinas - 03/02/2016
	aMsgAux := StrToKarr(cAux,"#")	
	For nI:=1 To Len(aMsgAux)
		cMens := aMsgAux[nI]
		
		nTMens := MLCount(cMens, MAXMENLIN)
		AADD(aMensagem,MemoLine(cMens, MAXMENLIN,1))
		For nJ:=2 To nTMens		            
			AADD(aMensagem,Memoline(cMens, MAXMENLIN,nJ))		
		Next nJ		
...
//INÍCIO CUSTOM. - Thiago Meschiatti FSW - Totvs IP 
...
//	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*25)+1),25), oFont12N:oFont )
	...
	nLinCalc+=10
Next nX
//FIM CUSTOM. - Thiago Meschiatti FSW - Totvs IP
...
If nFolha == 1
	...
	nFontSize := 28
	oDanfe:Code128C(074,365,SubStr(oNF:_InfNfe:_ID:Text,4), nFontSize )
EndIf
...
//INICO CUSTOM - CASSIANO
//aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont08:oFont, oFont08N:oFont)
aTamCol := RetTamCol(aAuxCabec, aAux, oDanfe, oFont07:oFont, oFont07N:oFont)
//FIM CUSTOM - CASSIANO
...
For nY := 1 To nLenItens
	...
	Else
		// INICIO CUSTOM - CASSIANO - Mundando a fonte de 8 para 7
		...
		nAuxH2 := len(aAux[1][7][nY]) + (nAuxH + (aTamCol[7]) - RetTamTex(aAux[1][7][nY], oFont07:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][7][nY], oFont07:oFont) // QUANT
		nAuxH += aTamCol[7]
		
		nAuxH2 := len(aAux[1][8][nY]) + (nAuxH + (aTamCol[8]) - RetTamTex(aAux[1][8][nY], oFont07:oFont, oDanfe))
		
		oDanfe:Say(nLinha, nAuxH2, aAux[1][8][nY], oFont07:oFont) // V UNITARIO
		nAuxH += aTamCol[8]
		
		nAuxH2 := len(aAux[1][9][nY]) + (nAuxH + (aTamCol[9]) - RetTamTex(aAux[1][9][nY], oFont07:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][9][nY], oFont07:oFont) // V. TOTAL
		nAuxH += aTamCol[9]

		nAuxH2 := len(aAux[1][10][nY]) + (nAuxH + (aTamCol[10]) - RetTamTex(aAux[1][10][nY], oFont07:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][10][nY], oFont07:oFont) // BC. ICMS
		nAuxH += aTamCol[10]
		
		nAuxH2 := len(aAux[1][11][nY]) + (nAuxH + (aTamCol[11]) - RetTamTex(aAux[1][11][nY], oFont07:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][11][nY], oFont07:oFont) // V. ICMS
		nAuxH += aTamCol[11]
		
		nAuxH2 := len(aAux[1][12][nY]) + (nAuxH + (aTamCol[12]) - RetTamTex(aAux[1][12][nY], oFont07:oFont, oDanfe))
		oDanfe:Say(nLinha, nAuxH2, aAux[1][12][nY], oFont07:oFont) // V.IPI
		nAuxH += aTamCol[12]
		
		nAuxH2 := len(aAux[1][13][nY]) + (nAuxH + (aTamCol[13]) - RetTamTex(aAux[1][13][nY], oFont07:oFont, oDanfe)) 
		oDanfe:Say(nLinha, nAuxH2, aAux[1][13][nY], oFont07:oFont) // A.ICMS
		nAuxH += aTamCol[13]
		
		nAuxH2 := len(aAux[1][14][nY]) + (nAuxH + (aTamCol[14]) - RetTamTex(aAux[1][14][nY], oFont07:oFont, oDanfe)) 
		oDanfe:Say(nLinha, nAuxH2, aAux[1][14][nY], oFont07:oFont) // A.IPI
		//FIM CUSTOM - CASSIANO
	EndIf
...
Static Function RetTamCol(aCabec, aValores, oPrinter, oFontCabec, oFont)
	...
	For nX := 1 To Len(aTamCol)
		...
		//INICIO CUSTOM. - Thiago Meschiatti FSW - 27/01/16
		if nX == 4 // CST
			aTamCol[nX] := aTamCol[nX]
		elseif nX == 5 // CFOP
			aTamCol[nX] := aTamCol[nX] 
		elseif nX == 7 // QUANT.
			aTamCol[nX] := aTamCol[nX]
		elseif nX == 8 // V.UNITARIO
			aTamCol[nX] := aTamCol[nX] + 20
		elseif nX == 9 // V. TOTAL
			aTamCol[nX] := aTamCol[nX] + 7
		elseif nX == 10 //BC. ICMS
			aTamCol[nX] := aTamCol[nX] + 15
		elseif nX == 11 //V.ICMS
			aTamCol[nX] := aTamCol[nX] + 6
		elseif nX == 12 //V.IPI
			aTamCol[nX] := aTamCol[nX] + 6
		elseif nX == 13 .or. nX == 14 // A.ICMS e A.IPI
			aTamCol[nX] := aTamCol[nX]
		endif
		//FIM CUSTOM. 
		nAux += aTamCol[nX]
