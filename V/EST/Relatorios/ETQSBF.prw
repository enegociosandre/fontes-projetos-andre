...
User Function ETQSBF()
	...
	Processa( {|| PrintETQ() },"imprimindo Etiqueta de Saldos..." )  
...
Static Function PrintETQ()
	...
	cQuery += "  BF_PRODUTO = B1_COD AND "
	cQuery += "  BF_PRODUTO >= '" + mv_par01 + "' AND BF_PRODUTO  <= '" + mv_par02 + "' AND "
	cQuery += "  BF_LOCAL   >= '" + mv_par03 + "' AND BF_LOCAL    <= '" + mv_par04 + "' AND "
	cQuery += "  BF_LOCALIZ >= '" + mv_par05 + "' AND BF_LOCALIZ  <= '" + mv_par06 + "' AND "
	cQuery += "  BF.D_E_L_E_T_ = ' '  AND "
	...
	While !(cAliasSBF)->(Eof())     
		...
		IncProc("Produto:" +Alltrim(SBF->BF_PRODUTO)+ " - Localização "+Alltrim(SBF->BF_LOCAL)+"-"+Alltrim(SBF->BF_LOCALIZ))   
		
		if (cAliasSBF)->B1_QE > 0 .and. (cAliasSBF)->BF_QUANT >= (cAliasSBF)->B1_QE 
		
			nQtdEtq	:= INT((cAliasSBF)->BF_QUANT / (cAliasSBF)->B1_QE)
			nQuant 	:= (cAliasSBF)->B1_QE	
			//nResto 	:= (((cAliasSBF)->BF_QUANT / (cAliasSBF)->B1_QE) - nQtdEtq ) * 100	
			nResto	:= Mod((cAliasSBF)->BF_QUANT, (cAliasSBF)->B1_QE)
		else
		
			nQtdEtq	:= 1
			nQuant 	:= (cAliasSBF)->BF_QUANT
			nResto 	:= 0
		endif	
		...
		For nX:= 1 to nQtdEtq
			...
			MSCBWrite("^FT16,265^A0N,25,24^FH\^FD"+Substr((cAliasSBF)->B1_DESC,1,40)+"^FS")		
			...
			MSCBWrite("^FD>:"+Alltrim((cAliasSBF)->B1_CODBAR)+"^FS")      
			MSCBWrite("^FT263,166^A0N,25,24^FH\^FDQtd / Emb^FS")
	   		MSCBWrite("^FT261,201^A0N,25,24^FH\^FD"+Alltrim(STR(nQuant))+' / '+Alltrim(STR((cAliasSBF)->B1_QE))+"^FS")
			MSCBWrite("^FT523,233^A0N,23,24^FH\^FDLocaliz.^FS")
			MSCBWrite("^FT524,266^A0N,23,24^FH\^FD"+(cAliasSBF)->BF_LOCAL+'-'+Alltrim((cAliasSBF)->BF_LOCALIZ)+"^FS")
			MSCBWrite("^FT264,123^A0N,23,24^FH\^FD"+Alltrim((cAliasSBF)->BF_PRODUTO)+"^FS")
...
Static Function ValidPerg(cPerg)  
	...
	aAdd(aRegs, {cPerg, "03", "Armazem de"		,"" ,"" ,"mv_ch3", "C", 02, 0, 0, "G", "", "mv_par03",  "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SBF"}) 
	aAdd(aRegs, {cPerg, "04", "Armazem ate"		,"" ,"" ,"mv_ch4", "C", 02, 0, 0, "G", "", "mv_par04",  "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SBF"}) 
	aAdd(aRegs, {cPerg, "05", "Endereço de"		,"" ,"" ,"mv_ch5", "C", 15, 0, 0, "G", "", "mv_par05",  "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SBF"}) 
	aAdd(aRegs, {cPerg, "06", "Endereço ate"	,"" ,"" ,"mv_ch6", "C", 15, 0, 0, "G", "", "mv_par06",  "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "SBF"}) 
