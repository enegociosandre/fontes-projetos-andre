...
If Pergunte("NFSIGW",.T.)
	...
	If !lImpDir
		...
		//cTipoNf  := SubString((cAliasSF3)->F3_CFO,1,1)                            
		//cZzNumer := IIF( cTipoNf >= "5",Posicione("SF2",1,xFilial("SF2")+(cAliasSF3)->F3_NFISCAL+(cAliasSF3)->F3_SERIE+(cAliasSF3)->F3_CLIEFOR+(cAliasSF3)->F3_LOJA,"F2_ZZNUMER"),"")
		If !lQuery
			DBClearFilter()
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(042,000,cLogoD,095,030)
	Else
		oDanfe:SayBitmap(042,000,cLogo,095,030)
	EndIF
Endif
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			If lMv_Logod
				oDanfe:SayBitmap(000+nLinhavers,000,cLogoD,095,030)
			Else
				oDanfe:SayBitmap(000+nLinhavers,000,cLogo,095,030)
			EndIf
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		If lMv_Logod
			oDanfe:SayBitmap(000,000,cLogoD,095,030)
		Else
			oDanfe:SayBitmap(000,000,cLogo,095,030)
		EndIf
