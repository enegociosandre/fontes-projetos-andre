		...
		Else
			...
			If !Empty(SF2->F2_TRANSP)
				...
				If !Empty(SF2->F2_VEICUL1)
					...
					aadd(aVeiculo,SF2->F2_ZZPLACA)
					aadd(aVeiculo,SF2->F2_ZZUFPLA)
					aadd(aVeiculo,"")//RNTC					
			...
			While ( !Empty(cScan) )
				...
				If !Empty(cEspecie)
					...
					If ( nScan==0 )
						aadd(aEspVol,{ cEspecie, FieldGet(FieldPos("F2_VOLUME"+cScan)) , SF2->F2_PLIQUI , SF2->F2_PBRUTO,SF2->F2_ZZMARCA,SF2->F2_ZZNUMER})
					Else
...
If Aviso(FunDesc(), cString, {"Transmitir","Cancelar"}, 3, "XML NFe") == 2
	cString := ""
EndIf
...
If cVerAmb== "2.00"
    ...
    cFoneDest += IIF(aTelDest[3] > 0,ConvType(aTelDest[3],18),"") // CÛdigo do Telefone
