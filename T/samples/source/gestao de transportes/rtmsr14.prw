...
Local aArea    := GetArea()
Local lEnd     := .T.
Local cDesNeg  := Posicione("DW5",1,xFilial("DW5")+DW7->DW7_CODNEG,"DW5_DESPRO")
Local aRodaPe  := {}
Local lRet     := .T.
...
//-- Nao considera proposta principal no reajuste
If DW7->DW7_TIPPRO == StrZero(2,Len(DW7->DW7_TIPPRO)) //-- Reajuste
	cNumPri := ""
EndIf
		...
		While (cAliasDWJ)->(!Eof())
			...
			While (cAliasDWJ)->(!Eof()) .And. (cAliasDWJ)->(DWJ_CDRORI+DWJ_CDRDES+DWJ_CODPRO+DWJ_SERVIC) == cCdrOri+cCdrdes+cCodPro+cServic
				...
				If (!lPraca .Or. Posicione("DUY",1,xFilial("DUY")+cRegPri,"DUY_CATGRP") <> StrZero(1,Len(DUY->DUY_CATGRP)))
					If (cAliasDWJ)->ORIGEM == "2"
						If (cAliasDWJ)->DWJ_CODPAS $ cCompFaiPri .Or. (cAliasDWJ)->DWJ_CODPAS $ "MN"
							If !( (cAliasDWJ)->DWJ_CODPAS $ cCompEstPri )
								cCompEstPri += "'" + (cAliasDWJ)->DWJ_CODPAS + "',"
							EndIf
							If (cAliasDWJ)->DWJ_CODPAS $ "MN"
						...
						EndIf
					Else
						If (cAliasDWJ)->DWJ_CODPAS $ cCompFai .Or. (cAliasDWJ)->DWJ_CODPAS $ "MN"
			...
			If !Empty(cCompEst)
				...
				cQuery += " 		 DWJ_CDRORI,DWJ_CDRDES,DWJ_CODPRO,DWJ_SERVIC, "
				cQuery += " 		 DWJ_RGOTAB,DWJ_RGDTAB,DWJ_PRDTAB,DWJ_CODPAS,DWJ_ITEM, "
				cQuery += " 		 DWJ_PERAJU,DWJ_VALATE,DWJ_INTERV, '1' ORIGEM, 0 DWD_VALMIN "
				cQuery += " FROM "
				cQuery += RetSqlName("DWJ")+" DWJ, "
				cQuery += RetSqlName("DUY")+" DUY "
				cQuery += "  WHERE DWJ_FILIAL = '"+xFilial("DWJ")+"'"
				...
				If !Empty(cNumPri)
					...
					cQuery += " 		 DWJ_CDRORI,DWJ_CDRDES,DWJ_CODPRO,DWJ_SERVIC, "
					cQuery += " 		 DWJ_RGOTAB,DWJ_RGDTAB,DWJ_PRDTAB,DWJ_CODPAS,DWJ_ITEM, "
					cQuery += " 		 DWJ_PERAJU,DWJ_VALATE,DWJ_INTERV, '2' ORIGEM, 0 DWD_VALMIN"
					cQuery += " FROM "
					cQuery += RetSqlName("DWJ")+" DWJ, "
					cQuery += RetSqlName("DUY")+" DUY "
					cQuery += "  WHERE DWJ_FILIAL = '"+xFilial("DWJ")+"'"
	...
	For nX := 1 To nTotBox
		...
		If aItensPro[1,5,nX,1] <> cCodPas
			...
			If lPerFai
				...
				lPerFai := .F.
			EndIf	
			//PcoPrtCell(nIniCell,nLin-60,PcoPrtTam(3)/nTotBox,60,If(lNumPri .And. cTabFre+cTipTab<>cTabOld+cTipOld,"Tabela 2 - " + cDesPas,cDesPas),oPrint,4,2,/*RgbColor*/,,,,,.T.) 
			PcoPrtCell(nIniCell,nLin-60,PcoPrtTam(3)/nTotBox,60,If(lNumPri ,"Tabela 2 - " + cDesPas,cDesPas),oPrint,4,2,/*RgbColor*/,,,,,.T.) 
		EndIf
		cTabOld := cTabFre
		cTipOld := cTipTab
		cDesFai := ""
