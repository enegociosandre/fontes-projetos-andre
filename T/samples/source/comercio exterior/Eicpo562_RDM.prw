...
IF GetMv("MV_ID_CLI") $ cSim
   //-----------> Cliente.
   SA1->( DBSETORDER( 1 ) )
   SA1->( DBSEEK( xFilial()+SW2->W2_CLIENTE ) )
ELSE
   //-----------> Comprador.
   SY1->( DBSETORDER( 1 ) )
      ...
      FOR I:=1 TO 2
          ...
          IF I==1
             IF EMPTY( SW2->W2_EXPORTA )
                ...
                LOOP
             ELSE
                SA2->( DBSEEK( xFilial()+SW2->W2_EXPORTA ) )
                oPrn:Say( Linha:=Linha+30 , 175  , STR0023,aFontes:TIMES_NEW_ROMAN_14_UNDERLINE ) //"** EXPORTADOR **"
                oPrn:Say( Linha:=Linha+80 , 175  , ALLTRIM(SW2->W2_EXPORTA)+" - "+ALLTRIM(SA2->A2_NOME),aFontes:TIMES_NEW_ROMAN_12)
             ENDIF
          ELSE
             SA2->( DBSEEK( xFilial()+SW2->W2_FORN ) )
             oPrn:Say( Linha:=Linha+30 , 175  , STR0025,aFontes:TIMES_NEW_ROMAN_14_UNDERLINE ) //"** FORNECEDOR **"
             oPrn:Say( Linha:=Linha+80 , 175  , ALLTRIM(SW2->W2_FORN)+" - "+ALLTRIM(SA2->A2_NOME),aFontes:TIMES_NEW_ROMAN_12)
          ENDIF
